...
def update_atm_status(status):
    ...
    if result:
        ...
        cursor.execute("UPDATE accounts SET atm = %s WHERE accno = %s", (status, account_number_val))
        connection.commit()

        messagebox.showinfo("Success", f"ATM has been {status.lower()}ed successfully.")
    else:
...
ATM_label = Label(t, text="ATM", font=("Arial", 30), bg="Turquoise", fg="brown")
ATM_label.place(relx=0.48, rely=0.15, anchor=CENTER)
...
activate_button = Button(t, text="Activate", font=("Arial", 20), bg="green", fg="white", command=lambda: update_atm_status("Activated"))
activate_button.place(relx=0.2, rely=0.65, anchor=CENTER)


deactivate_button = Button(t, text="Deactivate", font=("Arial", 20), bg="green", fg="white", command=lambda: update_atm_status("Deactivated"))
